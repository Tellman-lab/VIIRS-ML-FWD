...
class LateFusionUNetReg(nn.Module):
    def __init__(self, n_channels, n_classes):
        super(LateFusionUNetReg, self).__init__()
        self.n_channels = n_channels
        self.n_classes = n_classes

        self.firstStep = nn.Conv2d(3, 3, kernel_size=3, padding=1, padding_mode='reflect', stride=2)
        ...
        self.outc = nn.Sequential((OutConv(32, n_classes)), SigmoidRange(0,1))

    def forward(self, x):
        x0 = self.firstStep(x[0])
    ...
    def use_checkpointing(self):
        self.firstStep = torch.utils.checkpoint(self.firstStep)
